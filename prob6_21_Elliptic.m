...
% -------------------- Elliptic IIR (minimum order) --------------------
Wp = fpass/(fs/2);          % normalized passband edge
Ws = fstop/(fs/2);          % normalized stopband edge
[Ne, We] = ellipord(Wp, Ws, RpDB, RsDB);
[num_e, den_e] = ellip(Ne, RpDB, RsDB, We, 'low');
[He, fe] = freqz(num_e, den_e, 1024, fs);
fprintf(1, ['Elliptic IIR minimum order is ' num2str(Ne) '\n']);
...
% -------------------- Figure 2: Blowups of Pass/Transition/Stop Bands --------------------
figure(2); clf;

subplot(3,1,1);            % Pass band
plot(f,  abs(H),  'b'); hold on;
plot(fe, abs(He), 'r');
axis([0 fpass 1-Rpass 1+Rpass]);
title('Pass band');
xlabel('frequency in Hz'); ylabel('gain');
